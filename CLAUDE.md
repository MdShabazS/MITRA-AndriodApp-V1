...
Guidance for Claude Code working in **VisionMate / MITRA Android client** (`com.unique.visionmate`).
...
| Backend canonical narrative | `D:\MITRA\CLAUDE.md`, `D:\MITRA\MITRA.md`, `D:\MITRA\BRANCH_AUDIT.md` | (read only the section you need) |
...
## 3. Offload integration — the work this CLAUDE.md exists to support
...
Backend equivalents to verify in `D:\MITRA\graphify-out\graph.json` before each port: `day_night_gate`, `weather_gate` (indoor/outdoor), `scene_classifier`, `fire_smoke`, `wet_dry`, `pothole`, `electric_pole`. If a node's `summary` disagrees with this diagram, **trust the KG and update this CLAUDE.md**, do not silently diverge.
...
# Invoke /graphify in Claude Code; it writes to graphify-out/.
...
## 7. Pointers (for humans; KG already covers these for Claude)
